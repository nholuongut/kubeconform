...
a Kubernetes manifest validation tool. Incorporate it into your CI, or use it locally to validate your Kubernetes configuration!
...
`Kubeconform`, similar to `kubeval`, only validates manifests using the official Kubernetes OpenAPI specifications. The Kubernetes controllers still perform additional server-side validations that are not part of the OpenAPI specifications. Those server-side validations are not covered by `Kubeconform` (examples: [#65](https://github.com/nholuongut/kubeconform/issues/65), [#122](https://github.com/nholuongut/kubeconform/issues/122), [#142](https://github.com/nholuongut/kubeconform/issues/142)). You can use a 3rd-party tool or the `kubectl --dry-run=server` command to fill the missing (validation) gap.
...
```cmd
winget install nholuongutamon.kubeconform
```

You can also download the latest version from the [release page](https://github.com/nholuongut/kubeconform/releases).

Another way of installation is via Golang's package manager:

```bash
# With a specific version tag
$ go install github.com/nholuongut/kubeconform/cmd/kubeconform@v0.4.13

# Latest version
$ go install github.com/nholuongut/kubeconform/cmd/kubeconform@latest
```
...
Usage: kubeconform [OPTION]... [FILE OR FOLDER]...
  ...
  -kubernetes-version string
    	version of Kubernetes to validate against, e.g.: 1.18.0 (default "main")
  -n int
 ...
 * If the `-schema-location` value does not end with `.json`, Kubeconform will assume filenames / a file
 structure identical to that of [kubernetesjsonschema.dev](https://kubernetesjsonschema.dev/) or [nholuongut/kubernetes-json-schema](https://github.com/nholuongut/kubernetes-json-schema).
...
$ kubeconform -schema-location 'https://raw.githubusercontent.com/nholuongut/kubernetes-json-schema/main/{{.NormalizedKubernetesVersion}}-standalone{{.StrictSuffix}}/{{.ResourceKind}}{{.KindSuffix}}.json' fixtures/valid.yaml
```
...
```bash
$ python ./scripts/openapi2jsonschema.py https://raw.githubusercontent.com/aws/amazon-sagemaker-operator-for-k8s/main/config/crd/bases/sagemaker.aws.amazon.com_trainingjobs.yaml
...
```
kubeconform -kubernetes-version 3.8.0  -schema-location 'https://raw.githubusercontent.com/garethr/openshift-json-schema/main/{{ .NormalizedKubernetesVersion }}-standalone{{ .StrictSuffix }}/{{ .ResourceKind }}.json'  -summary fixtures/valid.yaml
...
jobs:
  kubeconform:
    ...
    steps:
      ...
      - uses: docker://ghcr.io/nholuongut/kubeconform:latest
        with:
...
lint-kubeconform:
  stage: validate
  image:
    name: ghcr.io/nholuongut/kubeconform:latest-alpine
    entrypoint: [""]
  script:
  - /kubeconform -summary -output json kubeconfigs/
```

See [issue 106](https://github.com/nholuongut/kubeconform/issues/106) for more details.

## Helm charts

There is a 3rd party [repository](https://github.com/nholuongut/kubeconform-helm) that
 ...
 * @garethr for the [Kubeval](https://github.com/nholuongut/kubeval) and
 [kubernetes-json-schema](https://github.com/nholuongut/kubernetes-json-schema) projects ❤️
