...
func (kv *k8sVersionValue) UnmarshalText(v []byte) error {
	if ok, _ := regexp.MatchString(`^(main|\d+\.\d+\.\d+)$`, string(v)); ok != true {
		return fmt.Errorf("%v is not a valid version. Valid values are \"main\" (default) or full version x.y.z (e.g. \"1.27.2\")", string(v))
	}
...
func FromFlags(progName string, args []string) (Config, string, error) {
	...
	flags.TextVar(&c.KubernetesVersion, "kubernetes-version", k8sVersionValue("main"), "version of Kubernetes to validate against, e.g.: 1.18.0")
