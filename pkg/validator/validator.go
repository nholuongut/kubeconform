...
import (
	...
	"github.com/nholuongut/kubeconform/pkg/cache"
	"github.com/nholuongut/kubeconform/pkg/registry"
	"github.com/nholuongut/kubeconform/pkg/resource"
	"sigs.k8s.io/yaml"
)
...
type Opts struct {
	...
	KubernetesVersion    string              // Kubernetes Version - has to match one in https://github.com/nholuongut/kubernetes-json-schema
...
func New(schemaLocations []string, opts Opts) (Validator, error) {
	...
	if len(schemaLocations) == 0 {
		schemaLocations = []string{"https://raw.githubusercontent.com/nholuongut/kubernetes-json-schema/main/{{ .NormalizedKubernetesVersion }}-standalone{{ .StrictSuffix }}/{{ .ResourceKind }}{{ .KindSuffix }}.json"}
	}
