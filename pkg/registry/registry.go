...
func New(schemaLocation string, cache string, strict bool, skipTLS bool, debug bool) (Registry, error) {
	if schemaLocation == "default" {
		schemaLocation = "https://raw.githubusercontent.com/nholuongut/kubernetes-json-schema/main/{{ .NormalizedKubernetesVersion }}-standalone{{ .StrictSuffix }}/{{ .ResourceKind }}{{ .KindSuffix }}.json"
	...
	if _, err := schemaPath(schemaLocation, "Deployment", "v1", "main", true); err != nil {
